...
- (void)viewDidLoad {
    ...
    [keqiangrightBtn setBackgroundImage:[UIImage imageNamed:@"yntymeassgeaeshk"] forState:UIControlStateNormal];
...
-(void)keqiangmsgTalevieheaderCLicks{
    MJWeakSelf;
    NSArray * keqiangData = [WHC_ModelSqlite query:[yntyHomeYundonModel class]];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.4 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (![keqiangboyState isEqualToString:@"1"]) {
            ...
            LYEmptyView * lyemtyVew = [LYEmptyView emptyActionViewWithImage:[UIImage imageNamed:@"yntynotlogin"] titleStr:@"未登录" detailStr:nil btnTitleStr:@"去登录" target:self action:@selector(keqiangemtvotl)];
        ...
        for (yntyHomeYundonModel * mdoel in keqiangData) {
            if (mdoel.isColltecd) {
        ...
        if (weakSelf.keqiangmsgDataArr.count == 0) {
            ...
            LYEmptyView * lyeView = [LYEmptyView emptyViewWithImage:[UIImage imageNamed:@"yntynotlogin"] titleStr:@"暂无数据" detailStr:nil];
