...
- (void)viewDidLoad {
    ...
    [keqiangrightBtn setBackgroundImage:[UIImage imageNamed:@"saomiao"] forState:UIControlStateNormal];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString * keqianghomeIdentifer = @"keqiangboyHomeTableViewCell";
    keqiangboyHomeTableViewCell * keqiangCell = [tableView dequeueReusableCellWithIdentifier:keqianghomeIdentifer];
    if (keqiangCell == nil) {
        keqiangCell =[[keqiangboyHomeTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:keqianghomeIdentifer];
    }
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    keqiangiconImgView.image = [UIImage imageNamed:@"wenzhang_huaban"];
...
-(void)keqiangboyHomeTableViewHeaderClicks{
    ...
    NSMutableDictionary * keqiangboyParmter = [[NSMutableDictionary alloc]initWithDictionary:@{@"pageNumber":@"2",@"pageSize":@"4",@"searchBefore":@"true"}];
    [TTJZHttpTool getHttpRequestWithURL:@"http://121.36.193.131/api/v3.1/competition/compleague/GetListForPopular" Parameters:keqiangboyParmter Success:^(id  _Nonnull object) {
        if (weakSelf.keqiangboyDataArr.count > 0) {
            [weakSelf.keqiangboyDataArr removeAllObjects];
        }
        ...
        if (weakSelf.keqiangboyDataArr.count == 0) {
            ...
            LYEmptyView * keqiangemtyview = [LYEmptyView emptyViewWithImage:[UIImage imageNamed:@"weidenglu"] titleStr:@"暂无推荐" detailStr:nil];
